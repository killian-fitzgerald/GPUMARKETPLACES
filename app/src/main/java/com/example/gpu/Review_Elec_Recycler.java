...
public class Review_Elec_Recycler extends RecyclerView.Adapter<electronicss>{
    List<data> electronicslist;
    private int lastposition = -1;
    Context context;

    public Review_Elec_Recycler(List<data> electronicslist, Context context) {
    ...
    @NonNull
    @Override
    public electronicss onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View myview = LayoutInflater.from(parent.getContext()).inflate(R.layout.row,parent,false);
        return new electronicss(myview);
    }

    @Override
    public void onBindViewHolder(@NonNull final electronicss holder, int position) {
        ...
        holder.cardView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(context,Review_electronics_detail.class);
                ...
                intent.putExtra("discount",electronicslist.get(holder.getAdapterPosition()).getDiscount());
                intent.putExtra("price",electronicslist.get(holder.getAdapterPosition()).getPrice());
                intent.putExtra("key",electronicslist.get(holder.getAdapterPosition()).getKey());
...
class electronicss extends RecyclerView.ViewHolder {
    TextView price,description,name;
    ImageView imageView;
    CardView cardView;


    public electronicss(@NonNull View itemView) {
        super(itemView);
